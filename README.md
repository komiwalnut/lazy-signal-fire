...
To run the script automatically at 00:00 UTC every day:
...
4. Add the following line (replace with your actual paths from steps 1 and 2):
   ```
   0 0 * * * cd /path/to/lazy-signal-fire && /path/to/pnpm start >> /path/to/lazy-signal-fire/cron.log 2>&1
   ```
   Example:
   ```
   0 0 * * * cd /home/ec2-user/lazy-signal-fire && /usr/local/bin/pnpm start >> /home/ec2-user/lazy-signal-fire/cron.log 2>&1
   ```
   This will run the script at exactly 00:00 UTC every day.
...
This will run the script at minute 0 of hour 0 (12:00 AM UTC) and save all output to `cron.log` in your project directory.
